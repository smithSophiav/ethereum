...
class ImportPrivateKeyActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent { EthereumTheme { ImportPrivateKeyScreen(etherWeb!!) } }
    }
...
@Composable
private fun ImportPrivateKeyScreen(etherWeb: EtherWeb) {
    ...
    Column(Modifier.fillMaxWidth().verticalScroll(rememberScrollState()).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Button(
            onClick = {
                ...
                val pk = privateKey.trim(); if (pk.isEmpty()) return@Button
                loading = true; resultText = ""; lastResult = null
                scope.launch {
                    val r = etherWeb.importAccountFromPrivateKeyAsync(pk)
                    loading = false
                    if (r != null) {
                        lastResult = r
                        resultText = "Address: ${r["address"]}\n\nPrivate Key: ${r["privateKey"]}"
                        isError = false
                    } else { resultText = "Invalid private key."; isError = true }
                }
            },
            modifier = Modifier.fillMaxWidth(),
            enabled = setupDone && !loading && privateKey.trim().isNotEmpty()
